...
// ── useDiveState (URL-fragment-persisted state) ────────────────────
...
function writeBag(bag: Record<string, unknown>) {
  ...
  const hash = Object.keys(bag).length ? `#state=${enc}` : "";
  window.history.replaceState(null, "", window.location.pathname + window.location.search + hash);
  window.dispatchEvent(new Event("divestate"));
}

export function useDiveState<T>(key: string, initialValue: T): [T, (v: T | ((p: T) => T)) => void] {
  const subscribe = useCallback((cb: () => void) => {
    window.addEventListener("divestate", cb);
    window.addEventListener("hashchange", cb);
    return () => {
      window.removeEventListener("divestate", cb);
      window.removeEventListener("hashchange", cb);
    };
  }, []);
  const getSnapshot = useCallback(() => {
    const bag = readBag();
    return key in bag ? JSON.stringify(bag[key]) : " ";
  }, [key]);
  const snap = useSyncExternalStore(subscribe, getSnapshot, getSnapshot);
  const value = snap === " " ? initialValue : (JSON.parse(snap) as T);
